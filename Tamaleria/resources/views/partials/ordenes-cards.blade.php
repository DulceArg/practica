                ...
                <div class="product-card-c" tabindex="0">
                    <figure class="card-banner-c">
                        ...
                        <ul class="card-action-list-c">
                            ...
                            <li class="card-action-item-c">
                                <form action="{{ route('ordenes.destroy', $orden) }}" method="POST" onsubmit="return confirm('¿Deseas eliminar esta orden?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <div class="card-content-c">
                        <div class="card-cat-c">
                            <a href="#" class="card-cat-link-c">{{ $orden->fecha_orden }}</a>
                        </div>
                        <h3 class="h3-c card-title-c">
                            <a href="#">{{ $orden->descripcion }}</a>
                        </h3>
                        <data class="card-price-c">${{ number_format($orden->total_orden, 2) }}</data>
                    </div>
                </div>
            </li>
        @endforeach
